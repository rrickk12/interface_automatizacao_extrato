...
import { runPipeline, uploadHTML } from './pipeline.js';
import {
  captureState,
  applyState,
  saveStateToServer,
...
import {
  ...
  carregarRegras,
  removerRegra,
  carregarRegrasConfig  // nova função exportada
} from './rules.js';
...
window.cleanState = function() {
  document.querySelectorAll("#aba-transacoes tbody tr").forEach(function(row) {
    ...
    row.className = "pending"; // estado padrão
  });
  cleanStateOnServer()
    .then(() => alert("State cleaned"))
    .catch(err => console.error(err));
};
...
window.adicionarRegra = function() {
  ...
  // Reaplica regras nas linhas pending
...
// ------------------------------
// Função para Upload de Extrato (HTML ou OFX)
// ------------------------------
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Carrega as regras do arquivo de configuração (config/rules.json)
  // Carrega as regras do endpoint /rules usando a função carregarRegrasConfig
  carregarRegrasConfig()
    .then(rulesFromConfig => {
      // Atribui as regras carregadas à variável global e atualiza a tabela de regras
      window.regras = rulesFromConfig;
      carregarRegras(window.regras, "#tabela-regras tbody");
      console.log("Regras carregadas do endpoint /rules:", window.regras);
      
      // Opcional: reaplica as regras nas transações pending
      document.querySelectorAll('#aba-transacoes tbody tr').forEach(row => {
        window.aplicarRegras(row);
      });
    })
    .catch(err => {
      console.error("Erro ao carregar regras do endpoint /rules:", err);
      window.regras = [];
    });
  ...
  if (tabelaRegras) {
    tabelaRegras.addEventListener('input', debounce(() => { window.saveRules(); }, 500));
  }
});
